...
mods_removed_raw=$(./pax/jd -set ./pax/modpack/manifest_prev.json $manifest | ggrep '^-' | ggrep -P -o '"name":[\s]*"\K[^"]*')
mods_added_raw=$(./pax/jd -set ./pax/modpack/manifest_prev.json $manifest | ggrep '^+' | ggrep -P -o '"name":[\s]*"\K[^"]*')
